...
<ul{if $level eq 0} id="jMenu"{/if}>{$first = 1}
    ...
    <li><a href="{if is_array($item.link)}{createUri controller=$item.link.controller action=$item.link.action params=$item.link.params}{else}{$item.link}{/if}" {if $level eq 0 and $first}class="fNiv"{$first = 0}{/if}>{$item.title}</a>{adminmenu menu=$item.sub level=$level+1}</li>
    {/foreach}
</ul>
{/if}
{/function}
