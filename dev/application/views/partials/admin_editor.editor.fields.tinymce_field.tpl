<div id="text_field_{$field->getField()}" class="highlight_field editor_field_line">
    ...
    <textarea class="tinymce" style="width: 100%;" rows="{$field->getNumberOfRows()}" name="data[{$field->getField()}]" id="{$field->getFieldHtmlID()}">{$smarty.post.data[$field->getField()]|default:$data[$field->getField()]|default:$field->getDefaultText()|escape:'html'}</textarea>
